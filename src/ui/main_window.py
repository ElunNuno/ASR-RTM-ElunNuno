...
class MainWindow(QMainWindow):
    ...
    def __init__(self, model_manager=None, config_manager=None):
        ...
        # 加载窗口状态
        print("config_manager._config type:", type(config_manager._config))
        print("config_manager._config keys:", getattr(config_manager._config, 'keys', lambda: [])())
        print("config_manager.get_config('window', default={}) =", config_manager.get_config('window', default={}))
    ...
    def _on_device_selected(self, device):
        ...
        if not self.control_panel.is_transcribing:
            ...
            # 滚动到顶部
            QTimer.singleShot(100, lambda: self.subtitle_widget.verticalScrollBar().setValue(0) if self.subtitle_widget.verticalScrollBar() else None)
    ...
    def set_recognition_language(self, language):
        ...
        if self.is_file_mode:
            ...
            # 更新菜单选中状态
            actions = getattr(self.menu_bar.transcription_menu, 'actions', None)
            if isinstance(actions, dict):
                if 'system_audio' in actions:
                    actions['system_audio'].setChecked(True)
                if 'file_audio' in actions:
                    actions['file_audio'].setChecked(False)
        ...
        if not self.control_panel.is_transcribing:
            ...
            self.subtitle_widget.subtitle_label.setText(info_text)
            # 滚动到顶部
            QTimer.singleShot(100, lambda: self._safe_scroll_to_bottom())
    ...
    def set_asr_model(self, model_name):
        ...
        if self.model_manager.load_model(model_name):
            ...
            # 更新字幕窗口的引擎类型
            try:
                setattr(self.subtitle_widget, 'current_engine_type', new_engine_type)
            except Exception:
                pass
    ...
    def show_asr_config_dialog(self):
        ...
        # 创建标签和输入框
        current_engine = getattr(self.model_manager, 'current_engine', None)
        config = getattr(current_engine, 'config', {}) if current_engine else {}
        if not isinstance(config, dict):
            config = {}
        enable_endpoint_label = QLabel("启用端点检测:")
        self.enable_endpoint_edit = QLineEdit(str(config.get("enable_endpoint", 1)))
        form_layout.addRow(enable_endpoint_label, self.enable_endpoint_edit)

        rule1_label = QLabel("规则1尾随静音时间 (秒):")
        self.rule1_edit = QLineEdit(str(config.get("rule1_min_trailing_silence", 3.0)))
        form_layout.addRow(rule1_label, self.rule1_edit)

        rule2_label = QLabel("规则2尾随静音时间 (秒):")
        self.rule2_edit = QLineEdit(str(config.get("rule2_min_trailing_silence", 1.5)))
        form_layout.addRow(rule2_label, self.rule2_edit)

        rule3_label = QLabel("规则3最小语音长度 (帧):")
        self.rule3_edit = QLineEdit(str(config.get("rule3_min_utterance_length", 25)))
    ...
    def save_asr_config(self):
        ...
        try:
            ...
            # 更新当前引擎的配置
            current_engine = getattr(self.model_manager, 'current_engine', None)
            config = getattr(current_engine, 'config', None)
            if not isinstance(config, dict):
                return
            for key, edit in zip([
                "enable_endpoint",
            ...
            ], [self.enable_endpoint_edit, self.rule1_edit, self.rule2_edit, self.rule3_edit]):
                value = edit.text()
                config[key] = value
    ...
    def load_window_state(self):
        ...
        try:
            ...
            try:
                # 获取窗口配置，强制传递 default 参数，防止 NoneType
                window_config = config_manager.get_config("window", default={})
                if not isinstance(window_config, dict):
                    window_config = {}
                ui_config = config_manager.get_config("ui", default={})
    ...
    def _on_file_selected(self, file_path):
        ...
        try:
            ...
            if isinstance(actions, dict):
                if 'system_audio' in actions:
                    actions['system_audio'].setChecked(False)
                if 'file_audio' in actions:
                    actions['file_audio'].setChecked(True)
    ...
    def _safe_scroll_to_bottom(self):
        scroll_bar = getattr(self.subtitle_widget, 'verticalScrollBar', lambda: None)()
        if scroll_bar:
